...
export function LiquidStakeDetail({ stakePool }: { stakePool: StakePool }) {
    ...
    return (
        <View tw="text-bold px-2" style={{
            ...
            <TabLayout stakePool={stakePool} solBalance={solbalance} tokenBalance={balance!} />
            {/* <View tw={`flex items-center mt-4`}>
...
const TabLayout = ({stakePool, tokenBalance, solBalance }: {stakePool:StakePool,tokenBalance: number, solBalance: number } )=> {
    const THEME = useCustomTheme();
            ...
            <View tw={`w-full flex flex-col items-center mt-6 h-full`}>
                {
                    ...
                    <View tw={`w-full`}>
                        ...
                        <Button onClick={() => depositSPLPool(stakePool, stakeAmount*LAMPORTS_PER_SOL)} tw="mt-60 w-full">Stake SOL</Button>
                    </View>
                }
                {
                    tabIndex === 1 &&
